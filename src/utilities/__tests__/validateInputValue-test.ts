...
describe('validateInputValue', () => {
  ...
  describe('for GraphQLInputObject', () => {
    ...
    it('returns error for an array type', () => {
      test([{ foo: 1 }, { bar: 1 }], TestInputObject, [
        {
          error:
            'Expected value of type "TestInputObject" to be an object, found: [{ foo: 1 }, { bar: 1 }].',
          path: [],
        },
      ]);
    });

    it('returns error for an array type on a nested field', () => {
      test({ foo: 1, deepObject: [1, 2, 3] }, TestInputObject, [
        {
          error:
            'Expected value of type "DeepObject" to be an object, found: [1, 2, 3].',
          path: ['deepObject'],
        },
      ]);
    });
