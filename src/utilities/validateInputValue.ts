...
function validateInputValueImpl(
  ...
  if (isListType(type)) {
    ...
    const knownFields: Array<string> = [];
    ...
    if (type.isOneOf) {
      ...
      const fieldKey = knownFields[0];
      const value = inputValue[fieldKey];
      if (value === null) {
        reportInvalidValue(
          onError,
          `Field "${fieldKey}" for OneOf type "${type}" must be non-null.`,
          path,
        );
      }
    }
